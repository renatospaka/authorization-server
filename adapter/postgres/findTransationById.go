...
func (p *PostgresDatabase) findTransationById(ctx context.Context, transactionId string) (*entity.Authorization, error) {
	log.Println("repository.authorizations.findTransationById")
	...
	rows := stmt.QueryRow(transactionId)
	var (
	...
	err = rows.Scan(&authorization_id, &client_id, &transaction_id, &status, &approved_at, &denied_at, &value, &created_at, &updated_at, &deleted_at)
	if err != nil {
